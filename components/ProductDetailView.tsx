    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 text-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12 bg-base-200/80 backdrop-blur-md p-8 rounded-2xl shadow-lg border border-white/10">
        ...
        <div className="flex flex-col">
          ...
          <p className="text-gray-300 leading-relaxed mb-8 flex-grow">{product.description}</p>
             ...
             <div className="flex items-center space-x-4 mb-4">
                <div className="flex items-center rounded-full bg-base-300/80 p-2">
                    <button
                        ...
                        className="p-2 rounded-full text-gray-300 hover:bg-base-100 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                        aria-label="Decrease quantity"
                    >
                        <MinusIcon className="w-6 h-6"/>
                    </button>
                    <span className="w-12 text-center font-bold text-2xl text-white tabular-nums">{quantity}</span>
                    <button
                        onClick={() => setQuantity(q => q + 1)}
                        disabled={!product.inStock}
                        className="p-2 rounded-full text-gray-300 hover:bg-base-100 disabled:opacity-50 transition-colors"
